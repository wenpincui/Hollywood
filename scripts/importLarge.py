...
conn = MySQLdb.connect('localhost','root','password','large')

cursor = conn.cursor()

txt = csv.reader(file('../data/large/movies.csv'))
line = 0
for row in txt:
    if (line > 0):
...
txt = csv.reader(file('../data/large/ratings.csv'))
line = 0
for row in txt:
    if (line > 0):
...
txt = csv.reader(file('../data/large/tags.csv'))
line = 0
for row in txt:
    if (line > 0):
...
txt = csv.reader(file('../data/large/links.csv'))
line = 0
for row in txt:
    if (line > 0):
